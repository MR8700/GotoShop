...
def seed_database():
    ...
    db = SessionLocal()
    try:
        if db.query(Store).count() >= 3 and db.query(SuperAdmin).count() > 0:
            return
        ...
        if not store_1:
            ...
            p1_1 = Product(
                ...
                name="Pagne Faso Danfani Traditionnel Tissé Main",
                slug="pagne-faso-danfani-traditionnel-3-pieces",
                description="Véritable Faso Danfani en pur coton burkinabè, tissé selon la tradition séculaire. Étoffe lourde, texture noble, teintes naturelles d'indigo et terre sahélienne. Vendu au pagne entier ou au demi-pagne (0,5).",
                short_description="100% pur coton burkinabè, tissé main à Koudougou. Choix par 0,5 ou 1 pagne.",
                price=15000,
                old_price=18000,
                currency="FCFA",
                stock=12.0,
                stock_label="12 pagnes disponibles",
                is_hero_deal=True,
                ...
                customization_prompt="Précisez les finitions souhaitées",
            )
            p1_2 = Product(
                store_id=store_1.id,
                ...
                currency="FCFA",
                stock=5.0,
                stock_label="Plus que 5 ensembles",
                is_hero_deal=False,
            ...
            p1_3 = Product(
                ...
                short_description="Coupe sirène impériale, Faso Danfani fin et dentelle noble. Sur mesure possible.",
                price=38000,
                old_price=48000,
                currency="FCFA",
                stock=4.0,
                ...
                customization_prompt="Indiquez vos mensurations (tour de poitrine, taille, hanches)",
            )
            p1_4 = Product(
                store_id=store_1.id,
                category_id=cat_accessoires_1.id,
                name="Tissu Faso Danfani au Mètre (Indigo Brut)",
                slug="tissu-faso-danfani-metre-indigo",
                description="Tissu Faso Danfani vendu à la coupe au mètre. Largeur standard 1,20 m. Idéal pour tailleurs, décorateurs d'intérieur et stylistes. Choisissez la longueur exacte désirée par tranche de 0,5 mètre.",
                short_description="Tissu Faso Danfani à la coupe, vendu au mètre linéaire (pas de 0,5 m).",
                price=3500,
                old_price=4500,
                currency="FCFA",
                stock=45.0,
                stock_label="45 mètres en rouleau",
                is_hero_deal=False,
                badge_tag="Vente au Mètre",
                active_discussions_count=14,
                views_count=520,
                sales_count=78,
                revenue=819000,
                guarantee_text="100% Tissé Main",
